...
//  g++ my_mink_glpk_algo2.cpp -L /usr/local/lib -l glpk -o my_mink_glpk_algo2
//  ./my_mink_glpk_algo2
...
int main() {
  ...
  std::string t1 = "cube5.ptop";
  qhullFile1 = t1;
  ptopformat1 = true;

  std::string t2 = "cube4.ptop";
  qhullFile2 = t2;
  ptopformat2 = true;
  ...
  {for (unsigned int u=1; u<=numberOfGenerators1; ++u) {
	...
	{for (unsigned int i=numberOfGenerators1+1; i<=numberOfGenerators1+numberOfGenerators2; ++i) {
	  if (ms_lp0.getSolutionVectorValue(i) != 0) {
	    ...
	    lp_sol2 += ms_lp0.getSolutionVectorValue(i)*matCol2;
	    //std::cout << "x" << i << "=" << ms_lp0.getSolutionVectorValue(i) << " ";
	  }
	}}
	...
	//std::cout << std::endl << std::endl;
      ...
      //else {
        ...
        //std::cerr << " z=" << z << " ; (x_u, x_v) = (" << ms_lp0.getSolutionVectorValue(u);
        //std::cerr << ", " << ms_lp0.getSolutionVectorValue(numberOfGenerators1+v) << ")" << std::endl;
