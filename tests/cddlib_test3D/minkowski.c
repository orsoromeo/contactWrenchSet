...
void callCdd()
{
  dd_MatrixPtr M2=NULL, A=NULL, G=NULL, sum=NULL;
  dd_PolyhedraPtr poly, polysum;
  dd_LPPtr lp;
  ...
  dd_set_global_constants();  /* First, this must be called. */
  	  ...
  	  dd_colrange n=3;    /* number of columns, dimension of the space */
  	  dd_MatrixPtr M=dd_CreateMatrix(m,n);

  	  dd_set_si(M->matrix[0][0], (long)1.0);
  	  dd_set_si(M->matrix[0][1], (long)0.0);
  	  dd_set_si(M->matrix[0][2], (long)1.0);

  	  dd_set_si(M->matrix[1][0], (long)0.0);
  	  dd_set_si(M->matrix[1][1], (long)1.0);
  	  dd_set_si(M->matrix[1][2], (long)1.0);

  	  dd_set_si(M->matrix[2][0], (long)2.0);
  	  dd_set_si(M->matrix[2][1], (long)2.0);
